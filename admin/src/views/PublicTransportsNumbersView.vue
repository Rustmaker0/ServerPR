<script setup>
import { computed, ref, reactive, onBeforeMount, watch } from 'vue'; // Добавлен reactive
...
const filteredPublicTransportsNumbers = computed(() => {
  let filtered = publicTransportsNumbers.value;

  // Быстрый поиск по всем полям
  if (filters.search) {
    const searchTerm = filters.search.toLowerCase();
    filtered = filtered.filter(item => {
      const transportName = publicTransportsById.value[item.public_transport]?.name?.toLowerCase() || '';
      const number = item.number?.toLowerCase() || '';
      const transportId = item.public_transport?.toString() || '';
      
      return transportName.includes(searchTerm) ||
             number.includes(searchTerm) ||
             transportId.includes(searchTerm);
    });
  }
  ...
  if (filters.number) {
    filtered = filtered.filter(item => 
      item.number?.toLowerCase().includes(filters.number.toLowerCase())
    );
  }
  ...
  if (filters.transport_type) {
    filtered = filtered.filter(item => 
      item.public_transport?.toString() === filters.transport_type
    );
  }
...
async function fetchPublicTransportsNumbers() {
  ...
  try {
    ...
    console.error('Ошибка при загрузке номеров транспорта:', error);
  } finally {
    loading.value = false;
  }
}
...
async function onUpdate() {
  try {
    ...
    ToEdit.value = { number: '', public_transport: '' };
  } catch (error) {
...
async function onAdd() {
  try {
    ...
    ToAdd.value = { number: '', public_transport: '' };
  } catch (error) {
  ...
  <div class="container-fluid">
    ...
    <div class="card mb-3">
      <div class="card-body py-2">
          ...
          <div class="col-auto">
            ...
            >
              ...
              <span v-if="hasActiveFilters" class="badge bg-light text-dark ms-1">
                {{ filteredPublicTransportsNumbers.length }}/{{ publicTransportsNumbers.length }}
              </span>
            </button>
          </div>
        ...
        <div v-if="showFilters" class="mt-3 pt-3 border-top">
          <div class="row g-3">
            <div class="col-md-4">
              <label class="form-label small">Номер транспорта</label>
              <input
                type="text"
            ...
            <div class="col-md-4">
              <label class="form-label small">Тип транспорта</label>
              <select class="form-select form-select-sm" v-model="filters.transport_type">
                <option value="">Все типы</option>
                <option :value="transport.id.toString()" v-for="transport in publicTransports" :key="transport.id">
                  {{ transport.name }}
                </option>
              </select>
            </div>
    ...
    <div v-if="isAdmin" class="p-2 mb-3"> 
      <div class="card">
        <div class="card-header bg-success text-white">
          <h6 class="mb-0"><i class="bi bi-plus-circle"></i> Добавить номер транспорта</h6>
        </div>
        <div class="card-body">
              ...
              <div class="col-auto"> 
                <div class="form-floating"> 
                  <select class="form-select" v-model="ToAdd.public_transport" required>
                    ...
                    <option :value="g.id" v-for="g in publicTransports" :key="g.id">
                      {{ g.name }}
                    </option>
                  </select> 
                  <label for="floatingInput">Тип ТС</label> 
    ...
    <div class="card">
      <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0"><i class="bi bi-123"></i> Номера общественного транспорта</h5>
      ...
      <div class="card-body p-0">
        <div class="table-responsive">
          <table class="table align-middle mb-0">
            ...
            <tbody>
              <tr v-for="item in filteredPublicTransportsNumbers" :key="item.id">
                <td>
                  <span class="fw-medium">{{ publicTransportsById[item.public_transport]?.name }}</span> 
                  <small class="text-muted ms-1">(ID: {{ item.public_transport }})</small>
                </td>
                <td>{{ item.number }}</td>
                <td v-if="isAdmin">
                  <div class="btn-group btn-group-sm">
          ...
          <div v-if="filteredPublicTransportsNumbers.length === 0" class="text-center text-muted py-4">
            <i class="bi bi-inbox display-6"></i>
            <p class="mt-2 mb-3">Номера транспорта не найдены</p>
