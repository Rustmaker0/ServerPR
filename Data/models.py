...
class PeopleInPublicTransport(models.Model):
    ...
    measuring = models.ForeignKey(Measuring, on_delete=models.CASCADE, verbose_name="Замер", null=True)
    sitting_place = models.FloatField("Количество сидячих мест в транспорте")
    standing_place = models.FloatField("Количество стоячих мест в транспорте")
    entering_people = models.IntegerField("Количество вошедших в транспорт людей")
    leaving_people = models.IntegerField("Количество вышедших из транспорта людей")
    ...
    def __str__(self):
        return f"{self.public_transport.name} №{self.transport_number} - {self.time} (в замере {self.measuring})"